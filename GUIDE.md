...
> ⚠️ **这份文档已过期，是 2026-05 课堂期版本的快照。** 文中的架构图与端口
> （前端 :3000 / FastAPI :8000 / LangGraph :2026）、「三智能体切换」以及
> 项目/文件夹/用例 CRUD 章节都已经不成立：
>
> - 现在是**一个通用测试助手**（`smart_test_agent`），能力由用户在「智能体装配」
>   页装配，不再有模式切换；
> - 端口是 **启动器 :5010 / LangGraph :5011 / FastAPI :5012 / WebUI :5013 / Playwright 执行器 :5015 / LightRAG :5014（按需）**；
> - 用例存储改成了 Markdown 文档（一个项目 = `workspace/default/cases/<项目名>.md`），
>   独立的项目/文件夹/用例表已删除，「项目管理」就是 `/cases` 的文档列表。
>
> **以 `CLAUDE.md`（改造说明与路由）、`PLATFORM.md`（架构梳理）、`EVAL.md`（测评）
> 为准。** 本文保留下来只为对照早期形态。
