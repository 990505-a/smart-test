...
**CRITICAL: PROJECT.md states "Next.js 16" but no such version exists.** The latest stable Next.js is 15.4.4 (released May 10, 2026). This must be corrected in PROJECT.md. Next.js 15 ships with React 19 and the App Router, which is what the project needs.

**Port discrepancy: PROJECT.md says LangGraph API on port 2026.** Standard convention is 2024. Verify which port the classroom code uses and standardize.
...
pip install "deepagents>=0.5.5"
pip install "langgraph>=0.4"
pip install "langchain-core>=0.3"

# LLM providers
pip install "langchain-deepseek>=0.1"
pip install "langchain-community>=0.3"  # For Doubao/Vision model integration

# RAG
pip install "lightrag-hku[postgres]>=0.1"  # PostgreSQL backend for LightRAG
pip install "raganything>=0.1"
...
npx shadcn@latest add button card input dialog sheet scroll-area separator avatar badge tabs
...
# Visualization
npm install @antv/g2plot  # or @antv/s2 for tabular reports
