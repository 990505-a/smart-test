"""Codebase agent tools: codebase-memory graph queries.

与用例生成智能体共用一条 stdio MCP 会话路径（cbm_call）。工具面向代码
分析场景：定位符号、追调用链、按全名读源码；grep/glob/read_file 等文件
工具由 DeepAgents 后端直接提供（cwd = 本次会话选中的仓库，真实路径），
不在此重复。

目标仓库由「代码图谱」页选中的仓库决定，经 configurable.workspace_path
传入；未选中或该仓库未建索引时给出降级提示（改用 grep/read_file 按真实
路径检索）。
"""
...
def _project() -> str:
    """当前会话目标仓库对应的图谱项目名；未选中返回空串。

    命名规则与 exe 一致，统一由 codebase_service.project_name 提供——过去
    这里和用例智能体的工具各写了一份等价的 slug 实现，改一处漏一处。
    """
    ...
    return project_name(repo) if repo else ""


def _degraded(kind: str) -> str:
    return (f"Error: 无法使用代码图谱{kind}。可能原因：本次会话未选中仓库，"
            "或该仓库尚未建索引。请改用 grep/glob/read_file 按真实路径直接检索；"
            "如需建库，建议用户在平台「代码图谱 → 索引与规则」为该仓库完成一次索引。")
...
async def trace_symbol(function_name: str, direction: str = "both", depth: int = 2) -> str:
    ...
    result = await codebase_service.cbm_call(
        ...
        {"project": project, "function_name": function_name,
         ...
         "depth": depth, "limit": 60, "format": "json"},
        timeout=90,
    )
...
async def read_symbol(qualified_name: str) -> str:
    ...
    result = await codebase_service.cbm_call(
        "get_code_snippet",
        {"project": project, "qualified_name": qualified_name, "format": "json"},
        timeout=60,
    )
    if not result.get("success"):
...
async def repo_architecture(aspects: str = "overview") -> str:
    ...
    result = await codebase_service.cbm_call(
        "get_architecture",
        {"project": project, "aspects": [aspect], "format": "json"}, timeout=90)
    if not result.get("success"):
...
async def graph_search(query: str, semantic: bool = False, limit: int = 20) -> str:
    ...
    if semantic:
        ...
        result = await codebase_service.cbm_call(
            "search_graph",
            {"project": project, "semantic_query": keywords or [query],
        ...
        result = await codebase_service.cbm_call(
            "search_graph",
            {"project": project, "name_pattern": query, "limit": limit, "format": "json"},
            timeout=60,
        )
    if not result.get("success"):
