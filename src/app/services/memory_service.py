...
_BUILTIN: list[MemoryModule] = [
    MemoryModule(
        ...
        seed=(
            ...
            "- 不确定的需求不要猜：逐条列出来问。\n\n"
            "## 自动化工程怎么放（目录约定）\n\n"
            "按**被测对象**分目录，机制与用例分开：`automation/<类型>/<被测对象>/`\n"
            "- `lib/` —— 机制与**这个对象**的知识（对象路径常量、操作封装、通信细节）；\n"
            "  改行为改这里。\n"
            "- `cases/<用例>/` —— 某条用例的正文、录制原稿 `raw/`、构建器 `build.py`、\n"
            "  生成物 `dist/`、门面 `case.md`。\n"
            "- `docs/` 按需读的专题；`tools/` 只读探针；`archive/` 历史现场（**不要读**）。\n"
            "- 交付物（用例文档 / 需求包 / 报告）放 `cases/`、`requirements/`、`reports/`，\n"
            "  **不要**塞进 automation。\n\n"
            "三条命令：**跑** = `unity_run_script(script_id=…)`（别直接跑 `dist/` 里的文件，\n"
            "那会绕过平台的存证、录像与失败现场）；**改** = 改 `lib/` 或 `cases/<用例>/` 的\n"
            "源码 → `python cases/<用例>/build.py` → 重跑；**看** = `cases/<用例>/case.md`\n"
            "（起跑线 / 目标 / 跑到哪了 / 上次为什么挂）。**`dist/` 是生成物，永不手改。**\n\n"
            "## 记忆怎么用\n\n"
            "- 常驻（本文件 + `MEMORY.md` 的铁律 + `USER.md`）只放**跨项目、每次都成立**的\n"
            "  东西 —— 它每轮全量注入，越厚越拖慢并带偏判断。\n"
            "- 具体到某款游戏/某个项目的知识，放那个项目的 `docs/`，用到再读。\n"
            "- 写新结论前先问一句「换个项目还成立吗」：成立 → 常驻；不成立 → 落到对应项目\n"
            "  的 `docs/`。\n\n"
            "## Unity 自动化（硬规则）\n\n"
            "- **平台永不触发 Unity 域重载**：进/退 Play、刷新资产、重编译脚本全部禁用\n"
            "  （工具层已拦截 `unity_editor(\"play\"/\"stop\"/\"refresh\")` 与常见 reload 触发片段）。\n"
            "- **Play 只能由用户手动点**：需要 Play 时直接告诉用户「请先在 Unity 里点 Play」，\n"
            "  等用户确认后再继续，不要自己想办法进 Play。\n"
            "- **平台不做复位、也不检查起跑线**（2026-09-22 起）：复位是改被测对象状态\n"
            "  的动作，一律由用户手动做；「在不在起点」也由用户自己把握 —— 平台只在运行输出里\n"
            "  打一行「这条用例要求从哪开始」。想核对就用 `unity_start_line(scene=…, \n"
            "  wait_for=…)`（只读、按需查，别当闸门反复轮询）。用例里的 `RESET = {…}`\n"
            "  是**给人和用例库的起跑线标注**。\n"
            "- **跑不通的用例要改，不是放弃**：看 `unity_run_script` 返回的 `failure.kind` ——\n"
            "  `case`（对象改名/等待不够/流程改版）照失败现场改用例再跑；`environment` 不要\n"
            "  动用例，去修环境或请用户处理；`timeout` 看最后卡在哪一步。同一份改 3 轮仍不过\n"
            "  就停下报结论。**不许为了变绿放宽/删掉断言**。跑通之后才 `unity_save_script`\n"
            "  （传 script_id 覆盖同一条）—— 平台只把验证过的内容标成 active。\n"
            "- **要跑用例时先跑，别先通读源码**：源码是几万行的工程，通读一遍要十几分钟，\n"
            "  而失败现场（`failure.txt` / `steps.jsonl` / 录像）本来就会告诉你卡在哪。\n"
            "  只有 `failure.kind == \"case\"` 才去读 `lib/` 与正文定位；`environment` 直接\n"
            "  停下报环境问题。用户说「接着上次跑」时，先看 `unity_list_scripts` 里的\n"
            "  `last_run`（含 `env_suspect`），别去观测活着的游戏界面猜进度。\n"
            "- **跑完顺手更新 `case.md`**：起跑线、跑到哪、这次为什么挂、下一步 —— 那是\n"
            "  下一次会话（或下一个人）的起点。\n"
        ),
    ),
    MemoryModule(
        ...
        seed=(
            "# MEMORY.md — 长期记忆（常驻部分）\n\n"
            "> 这里只放**每次都要遵守**的铁律与**索引**：跨项目、短、立刻用得上。\n"
            "> 具体到某款游戏/某个项目的知识别写这里 —— 放那个项目的 `docs/`，用到再读。\n"
            "> 每条带上日期与来源，便于判断是否过期。\n"
        ),
    ),
    MemoryModule(
