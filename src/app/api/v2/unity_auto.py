"""Unity UI automation routes (Unity 自动化模块)."""
...
@router.post("/scripts", response_model=SuccessResponse, status_code=201, summary="创建 Unity 脚本")
async def create_script(data: ScriptCreate, user: CurrentUserDep, db: DbSessionDep):
    script = UnityScript(**data.model_dump())
    db.add(script)
    await db.commit()
    return SuccessResponse(success=True, data=_script_dict(script, full=True))


@router.get("/scripts", response_model=SuccessResponse, summary="Unity 脚本列表")
async def list_scripts(user: CurrentUserDep, db: DbSessionDep):
    rows = list((await db.execute(
        select(UnityScript).order_by(UnityScript.updated_at.desc()).limit(100)
    )).scalars().all())
    return SuccessResponse(success=True, data=[_script_dict(s) for s in rows])


@router.get("/scripts/{script_id}", response_model=SuccessResponse, summary="Unity 脚本详情")
...
@router.put("/scripts/{script_id}", response_model=SuccessResponse, summary="更新 Unity 脚本")
...
async def _run_and_record(script_id: str) -> None:
    ...
    async with async_session_factory() as db:
        ...
        if script is None:
            return
        run = UnityScriptRun(script_id=script.id)
        db.add(run)
        await db.flush()
        try:
            result = await unity_service.run_unity_script(str(script.id), script.name, script.content)
...
@router.post("/scripts/{script_id}/run", response_model=SuccessResponse, summary="执行 Unity 脚本(后台)")
...
@router.get("/scripts/{script_id}/runs", response_model=SuccessResponse, summary="Unity 脚本执行历史")
async def list_runs(script_id: str, user: CurrentUserDep, db: DbSessionDep):
    rows = list((await db.execute(
        select(UnityScriptRun).where(UnityScriptRun.script_id == UUID(script_id))
        .order_by(UnityScriptRun.created_at.desc()).limit(30)
    )).scalars().all())
